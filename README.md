...
A tree-based planning system that uses an LLM as a one-step action predictor to solve Sokoban puzzles. The LLM (Qwen2.5-7B-Instruct via vLLM) guides search by predicting the best next action at each board state. Three search algorithms are implemented: BFS (baseline), Beam Search, and A*.
...
├── search.py                   # BFS, Beam Search, A* solvers
...
├── compare_solvers.py          # Compare BFS vs Beam Search vs A* side-by-side
...
### 1. Full evaluation (BFS vs A*)
```bash
python3 evaluation.py
```
Runs both solvers on 10 varied-difficulty puzzles and prints a comparison table + saves plots to `outputs/`.

### 2. Compare search strategies (BFS vs Beam vs A*)
```bash
